...
* Rotate the calculated centroid coordinates relative to "north", as defined by `nstar_X` and `nstar_Y`.
* Convert cartesian coordinates of rotated coaptation line to spherical coordinates
...
D <-
  ...
  cbind(cart2sph(.$centroidCalcRotatedX, .$centroidCalcRotatedY, 0,
...
message(sprintf("Correlation between coapt_line_length and calculated radial is: %.05f",
                cor(D$coapt_line_length, D$radial, use = "complete.obs")))
```
```{r}
