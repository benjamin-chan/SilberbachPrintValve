...
pred <- 
  ...
  rename(predX = centroidBottomRotatedX,
         predY = centroidBottomRotatedY,
         predZ = centroidBottomZ) %>% 
  # unique %>% 
  bind_cols(., 
            sphereplot::car2sph(.$predX, .$predY, .$predZ) %>% data.frame,
            df %>% select(centroidBottomRotatedX, centroidBottomRotatedY, centroidBottomZ))
pred
```
