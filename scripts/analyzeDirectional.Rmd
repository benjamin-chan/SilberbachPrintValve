...
Spherical coordinates of coaptation lines: [PNG](../figures/plotSphericalCoordinates.png), [SVG](../figures/plotSphericalCoordinates.svg)

![Spherical coordinates](../figures/plotSphericalCoordinates.png)

```{r plotSphericalCoordinates, dpi = 300, fig.show = "hide"}
df %>%
  select(matches("^id|^type$|^(lat|long|magn)itude$")) %>%
  ggplot +
  aes(x = longitude, y = -latitude, size = magnitude, color = type) +
  geom_point(alpha = 1/2) +
  scale_color_brewer("", palette = "Set1") +
  scale_x_continuous("Longitude", breaks = seq(0, 360, 30), limits = c(0, 359)) +
  scale_y_continuous("\u2212Latitude", breaks = seq(0, 90, 10), limits = c(0, 90)) +
  scale_size_area("Coaptation\nline length\n(mm)") +
  coord_polar(start = -pi / 2, direction = -1) +
  theme_bw() +
  theme(panel.grid.minor = element_blank(),
        plot.title = element_text(hjust = 0.5))
ggsave("../figures/plotSphericalCoordinates.png", dpi = 300)
ggsave("../figures/plotSphericalCoordinates.svg", dpi = 300)
```
